package com.iflytek.mvvm_module_cli.model
...
import com.iflytek.mvvm_module_cli.net.AppService
...
open class AppMainModel(var netManger: NetManager, var userDao: UserDao) {
    ...
    fun getUserDetail(): Single<User>? {
        return userDao.getAllUser().map {
            ...
            netManger.getService(AppService::class.java)?.detail()
                ?.httpDataFilter()
                ?.map {
                    return@map User(it[0].name)
                }
        }
    }
